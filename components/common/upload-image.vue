					...
					success: res => {
						if(res.confirm){
							uniCloud.callFunction({
								name : 'delete-file',
								data : {
									fileID : _this.imageList[index]
								},
								success(res){
									console.log(res)
									_this.imageListType.splice(index,1)
			...
			chooseImage: async function() {
				...
				uni.chooseImage({
					...
					success: (res) => {
						let type = '';
						// #ifdef APP-PLUS
						type = res.tempFiles[0].path.split('.')[1] + ''
						// #endif
						// #ifdef H5
						type = res.tempFiles[0].type.split('/')[1]
						// #endif
						this.imageListType = this.imageListType.concat(type)
						this.imageList = this.imageList.concat(res.tempFilePaths);
						this.$emit('change',this.imageList,this.imageListType)
					},
					fail: (err) => {
						// #ifdef APP-PLUS
